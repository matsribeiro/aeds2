...
Movie movie[MAXTAM]; // Elementos da pilha
...
//========================SELEÇÃO RECURSIVA=========================================
void swap(Movie movie[], int i, int j)
{
   Movie aux = movie[i];
   movie[i] = movie[j];
   movie[j] = aux;
...
void quicksort(Movie movie[], int i, int j)
{
    int local = i;

    for(int n = i + 1; n < j; n++){
        if(strcmp(movie[n].situation, movie[local].situation) < 0){
            local = n;
        }
    }
