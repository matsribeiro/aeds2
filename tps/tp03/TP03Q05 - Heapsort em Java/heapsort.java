...
public class heapsort {
    ...
    public static void heapsort(ListaSeq lista){

        for(int i = 0; i < lista.getSize(); i++){
            int menor = i;
            for(int j = i+1; j < lista.getSize(); j++){
                if(lista.array[menor].getGenre().trim().compareToIgnoreCase(lista.array[j].getGenre().trim()) > 0){
                    menor = j;
                }else if(lista.array[menor].getGenre().trim().compareToIgnoreCase(lista.array[j].getGenre().trim()) == 0){
                    if(lista.array[menor].getGenre().trim().compareToIgnoreCase(lista.array[j].getGenre().trim()) > 0)
                        menor = j;
                }
            }
