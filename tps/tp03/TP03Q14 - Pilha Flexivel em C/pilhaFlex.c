...
Movie movie_clone(Movie *movie)
{
    ...
    for (int i = 0; i < movie->count_keywords; i++)
        strcpy(cloned.keywords[i], movie->keywords[i]);
    return cloned;
}
...
Celula *novaCelula(Movie elemento)
{
...
void mostrarRecursivo(int j, Celula *i)
{
    j--;
    if(i->prox != NULL){
        mostrarRecursivo(j, i->prox);
    }
...
int main()
{
    ...
    while (!isFim(line))
    {
        ...
        movie_start(&clone_movie);
        movie_readHtml(&clone_movie, line);
        inserir(clone_movie);
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        Movie movie;
        char lineAction[1000];
        char movieName[1000];
        char pos[1000];
        char namePos[1000];

        scanf(" %[^\n]", lineAction);
        strcpy(movieName, substring(lineAction, 3, strlen(lineAction)));

        if (lineAction[0] == 'I')
        {

            movie_start(&movie);
            movie_readHtml(&movie, movieName);
            inserir(movie);
        }
        else if (lineAction[0] == 'R')
        {
