...
public class mergesort {
    ...
    public static void mergesort(ListaSeq lista){

        for(int i = 0; i < lista.getSize(); i++){
            int menor = i;
            for(int j = i+1; j < lista.getSize(); j++){
                if(lista.array[menor].getBudget() > lista.array[j].getBudget()){
                    menor = j;
                }else if(lista.array[menor].getBudget() == lista.array[j].getBudget()){
                    if(lista.array[menor].getBudget() < lista.array[j].getBudget())
                        menor = j;
                }
            }
