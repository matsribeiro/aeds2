...
public class quicksort {
    ...
    public static void quicksort(ListaSeq lista){

        for(int i = 0; i < lista.getSize(); i++){
            int menor = i;
            for(int j = i+1; j < lista.getSize(); j++){
                if(lista.array[menor].getSituation().trim().compareToIgnoreCase(lista.array[j].getSituation().trim()) > 0){
                    menor = j;
                }else if(lista.array[menor].getSituation().trim().compareToIgnoreCase(lista.array[j].getSituation().trim()) == 0){
                    if(lista.array[menor].getSituation().trim().compareToIgnoreCase(lista.array[j].getSituation().trim()) > 0)
                        menor = j;
                }
            }
    ...
    public static void swap(int a, int b, Film array[]){
        Film tmp = array[a];
        array[a] = array[b];
        array[b] = tmp;
    }
