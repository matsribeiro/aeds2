    ...
    public static void selectionSort(ListaSeq lista){
        ...
        for(int i = 0; i < lista.getSize(); i++){
            ...
            for(int j = i+1; j < lista.getSize(); j++){
                if(lista.array[menor].getOgTitle().trim().compareToIgnoreCase(lista.array[j].getOgTitle().trim()) > 0){
                    menor = j;
                }else if(lista.array[menor].getOgTitle().trim().compareToIgnoreCase(lista.array[j].getOgTitle().trim()) == 0){
                    if(lista.array[menor].getOgTitle().trim().compareToIgnoreCase(lista.array[j].getOgTitle().trim()) > 0)
                        menor = j;
                }
            }
